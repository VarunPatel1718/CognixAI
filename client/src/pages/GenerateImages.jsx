    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)}
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-600/20 text-green-300 border-green-500/30' : 'text-slate-400 border-slate-700'}`}
              key={item}
            >
              {item}
            </span>
      ...
      rounded-full transition peer-checked:translate-x-4'></span>
        ...
        <button 
          type="submit"
          disabled={loading}
          className='w-full flex justify-center items-center gap-2 text-white px-4 py-3 mt-6 text-sm rounded-xl cursor-pointer font-medium disabled:opacity-60'
      ...
      <div className='w-full max-w-lg p-4 rounded-xl flex flex-col' style={{
        ...
        <div className='flex items-center gap-3'>
          ...
          <h1 className='text-xl font-semibold text-slate-200'>Generated images</h1>
        </div>
          ...
          {!loading && !error && content && (
            <div className='w-full space-y-4'>
              ...
              <button
                ...
                className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-gray-700 to-gray-900 text-white px-4 py-2 text-sm rounded-lg cursor-pointer hover:opacity-90 transition-opacity'
              >
